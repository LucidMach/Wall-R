...
const SignIn: React.FC = () => {
  return (
    <div className="w-full h-full flex flex-col justify-center items-center">
      ...
      <button
        onClick={handleClick}
        className="group flex items-center p-0.5 rounded-sm justify-between bg-blue-500 hover:bg-blue-600 active:bg-blue-400"
      >
        ...
        <span className="font-semibold text-lg text-white mx-8">
          Sign In with Google
        </span>
      </button>
      <h1 className="text-xs text-slate-400">
        we&apos;re working on integrating other auth providers
      </h1>
    </div>
  );
};
